...
The AST gives access to methods on nodes of the tree. The [`Raw`] node provides
3 methods that specify its abstract representation: [`Raw::lines()`] returns the
raw text as an iterator of lines, [`Raw::lang()`] provides the optionally
present [`RawLang`] language tag, and [`Raw::block()`] gives a bool for whether
the raw element is a block or inline.

These methods are implemented by assuming the structure of [`SyntaxNode`]s will
match what is generated by the parser. The [`Raw`] struct really is just a
wrapper over a [`SyntaxNode`] reference. But to construct the wrapper and access
the methods, we must check that the [`SyntaxKind`] of the underlying node
matches.
...
inline (although it will still be parsed into nodes). This is also a factor of
the current "tree-interpreter" evaluation model. A bytecode interpreter might
instead eagerly convert the AST into bytecode, but it will still traverse using
this lazy interface.

## The AST is rooted in the [`Markup`] node

Finally, the AST is rooted in the [`Markup`] node, which provides only one
method: [`Markup::exprs`]. This returns an iterator of the main [`Expr`] enum.
[`Expr`] is important because it contains the majority of expressions that Typst
will evaluate. Not just markup, but also math and code expressions. Not all
...
macro_rules! node {
    ($(#[$attr:meta])* struct $name:ident) => {
        ...
        // Implement `AstNode` for the struct.
